...
createRowAndAppen(item)
    }

  }
  function createRowAndAppen(item){
...
let [imgRef, nameRef, priceRef, decorFef] = Array.from(checkbox.parentElement.parentElement.children);
let nameValue = nameRef.children[0].textContent;
let priceValue = Number(priceRef.children[0].textContent);
let decFactorValue = Number(decorFef.children[0].textContent);
